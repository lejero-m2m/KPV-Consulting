...
If the user is **Kyle Dickinson** or the work is for **Karaka Pines Villages (KPV)**, read `references/kpv-conventions.md` first. It locks village register, lifecycle phases, sheet naming, brand palette, typography, documentation suite pattern, platform stack, org structure (§16), approval matrix (§17), and data ownership matrix (§18) — and overrides anything in the generic guidance below where there's a conflict. For the detailed naming standard (column names, cross-sheet reference abbreviations, AUTO_NUMBER ID formats, picklist value sets, automation naming) read `references/kpv-naming-conventions.md`. KPV is on Smartsheet Business plan, North American region — web app `app.smartsheet.com` and API `api.smartsheet.com` (confirmed 2026-05-22 inventory; the `.au` host is not used) — with 107 Papamoa as the trial village. Most of the KPV team never opens Smartsheet directly — design every output to **surface into Microsoft Teams**.
...
- `references/kpv-conventions.md` — **KPV-specific authoritative conventions**: village register (100–107), lifecycle phases, folder structure, naming conventions, locked sheet names, brand palette (`#1A2332` navy + Arial), documentation suite pattern (Flow PNG + Word manual), mapping conversation workflow, Teams channel mapping, KPV principles, org structure (§16), approval matrix (§17), data ownership matrix (§18), parked decisions. **Overrides generic guidance for KPV work.**
- `references/kpv-naming-conventions.md` — **detailed KPV naming standard** (added 2026-05-22): column name format, cross-sheet reference abbreviations (UR, SR, RR, ConP, CivP, ConR, ContR, VL, RFQ, HSI, HSM, TR, SuppR, PF), AUTO_NUMBER ID formats (4-digit padding), locked picklist value sets (Sales Status, RAG, H&S Incident Type, etc.), Quote ID derivation, Trade Package codes, automation naming, SharePoint file naming. **Authoritative for all KPV naming decisions** — supersedes the high-level §6 summary in `kpv-conventions.md`.
