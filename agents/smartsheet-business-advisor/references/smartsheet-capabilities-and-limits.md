...
| API token authenticates against `api.smartsheet.com`, not `api.smartsheet.au` | 107 Papamoa Budget build | Web app is `app.smartsheet.au`; the API endpoint is the global `api.smartsheet.com`. Use `api.smartsheet.com` for all scripts/MCP/API. Web-app region ≠ API region — do not infer one from the other. |
